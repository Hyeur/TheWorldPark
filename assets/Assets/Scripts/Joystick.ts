...
import { GameManager } from "./GameManager";
  ...
  export class Joystick extends Component {
    ...
    _initTouchEvent() {
      ...
      input.on(Input.EventType.TOUCH_START, this._touchStartEvent, this);
      input.on(Input.EventType.TOUCH_MOVE, this._touchMoveEvent, this);
      input.on(Input.EventType.TOUCH_END, this._touchEndEvent, this);
      input.on(Input.EventType.TOUCH_CANCEL, this._touchEndEvent, this);
    }

    start() {
        const playerCarController = GameManager.instance.playerCarController; // Store in a temp variable
        if (playerCarController) {
            playerCarController.joystick = this.node.getComponent(Joystick);
        }
    }
    ...
    _touchStartEvent(event: EventTouch) {
      ...
      this.node.emit(Input.EventType.TOUCH_START, event);
    ...
    _touchMoveEvent(event: EventTouch) {
      ...
      //this.node.emit('JoystickMove', moveVec.normalize());
      this.outMove = new Vec2(moveVec.normalize().x, moveVec.normalize().y);
    ...
    _touchEndEvent(event: EventTouch) {
      ...
      this.node.emit(Input.EventType.TOUCH_END, event, {
        speedType: SpeedType.STOP,
      });
      this.outMove = Vec2.ZERO;
    }
  }
